using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;
using SklepUKW.DAL;

namespace SklepUKW.Infrastructure
{
    public class CartManager
    {
        FilmsContext db;
        ISessionManager session;

        public CartManager(FilmsContext db, ISessionManager session)
        {
            this.db = db;
            this.session = session;
        }

        public List<CartItem> GetCartItems()
        {
            List<CartItem> items;

            if (session.Get<List<CartItem>>(Consts.CartSessionKey) == null)
            {
                // jak w sesji nic nie ma to tworzymy nowa
                items = new List<CartItem>();
            }
            else
            {
                // jak jest to wykorzystujemy
                items = session.Get<List<CartItem>>(Consts.CartSessionKey);
            }

            return items;
        }

        public void AddToCart(int filmID)
        {
            var cart = GetCartItems();
            var thisFilm = cart.Find(f => f.Film.FilmID == filmID);

            if (thisFilm != null)
            {
                // jesli juz jest w koszyku, to zwiekszyc wartosc
                thisFilm.Quantity++;
            }
            else
            {
                // jesli nie ma to stworz nowy item w koszyku z id z bazy danych
                // jesli id z bazy i przekazane sie zgadzaja, to stworz i dodaj do koszyka z odpowiednimi parametrami
                var newCartItem = db.Films.Where(f => f.FilmID == filmID).SingleOrDefault();
                if (newCartItem != null)
                {
                    var cartItem = new CartItem()
                    {
                        Film = newCartItem,
                        Quantity = 1,
                        Price = newCartItem.Price
                    };

                    cart.Add(cartItem);
                }
            }
            session.Set(Consts.CartSessionKey, cart);        
        }

        public int RemoveFromCart(int filmID)
        {
            var cart = GetCartItems();
            var thisFilm = cart.Find(f => f.Film.FilmID == filmID);

            if(thisFilm != null)
            {
                if(thisFilm.Quantity > 1)
                {
                    // jesli jest i wiecej niz 1 to zmniejszamy o 1
                    thisFilm.Quantity--;
                    return thisFilm.Quantity;
                }
                else
                {
                    cart.Remove(thisFilm);
                }
            }
            return 0;
        }

        public int GetCartQuantity()
        {
            var cart = GetCartItems();

            var sum = cart.Sum(i => i.Quantity);

            return sum;
        }

        public decimal GetCartValue()
        {
            var cart = GetCartItems();

            var sum = cart.Sum(i => (i.Price * i.Quantity));

            return sum;
        }
    }
}